/**************************************************************
  ...
  Notes:
    Converted nyanion failure on invalid character class 
...
class CEmit
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(null != spec);
	    CUtility.nyan(null != outstream);
	  }
  ...
  /*void emit_imports
      ...
      throws java.io.IOException      
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.nyan(null != m_spec);
	      CUtility.nyan(null != m_outstream);
	    }*/
  ...
  private void emit_table
	...
	for (elem=0; elem<size; elem++) {
	    ...
	    CUtility.nyan(dtrans.m_dtrans.length==m_spec.m_dtrans_ncols);
  ...
  private void emit_actions 
      ...
      throws java.io.IOException
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.nyan(m_spec.m_accept_vector.size() 
...
class CMakeNfa
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(null != input);
	    CUtility.nyan(null != lexGen);
	    CUtility.nyan(null != spec);
	  }
  ...
  void allocate_BOL_EOF
	...
	{
	  CUtility.nyan(CSpec.NUM_PSEUDO==2);
	...
	for (reduced_ncols = 0; ; ++reduced_ncols)
	  {
		...
		for (i = 0; i < reduced_ncols; ++i)
		  {
		    CUtility.nyan(-1 != m_spec.m_col_map[i]);
		  }
	      }
	...
	for (reduced_nrows = 0; ; ++reduced_nrows)
	  {
		...
		for (i = 0; i < reduced_nrows; ++i)
		  {
		    CUtility.nyan(-1 != m_spec.m_row_map[i]);
		  }
	      }
	...
	while (old_group_count != group_count)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.nyan(m_group.size() == group_count);
	      }
		...
		for (j = 1; j < group_size; ++j)
		  {
			...
			if (goto_first != goto_next
				...
				|| m_ingroup[goto_next] != m_ingroup[goto_first]))
			  {
			    if (CUtility.DEBUG)
			      {
				CUtility.nyan(dtrans_group.elementAt(j) == next);
			      }
			    ...
			    if (CUtility.DEBUG)
			      {
				CUtility.nyan(m_group.contains(new_group)
						== true);
				CUtility.nyan(m_group.contains(dtrans_group)
						== true);
				CUtility.nyan(dtrans_group.contains(first)
						== true);
				CUtility.nyan(dtrans_group.contains(next)
						== false);
				CUtility.nyan(new_group.contains(first)
						== false);
				CUtility.nyan(new_group.contains(next)
						== true);
				CUtility.nyan(dtrans_group.size() == group_size);
				CUtility.nyan(i == m_ingroup[first.m_label]);
				CUtility.nyan((m_group.size() - 1) 
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.nyan(i == dtrans.m_label);
		CUtility.nyan(false == group_found);
		CUtility.nyan(group_count == m_group.size());
	      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.nyan(false == group_found);
		    CUtility.nyan(0 < dtrans_group.size());
		  }
		...
		if (CUtility.SLOW_DEBUG)
		  {
			...
			CUtility.nyan(check.m_accept == first.m_accept);
		      }
		  }
		...
		if (first.m_accept == dtrans.m_accept)
		  {
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.nyan(j == m_ingroup[dtrans.m_label]);
		      }
...
class CNfa2Dfa
{
	...
	for (istate = 0; nstates > istate; ++istate)
	  {
	    ...
	    */
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.nyan(false == dfa.m_mark);
		  }
		...
		for (i = 0; i < m_spec.m_dtrans_ncols; ++i)
		  {
		    if (CUtility.DEBUG)
		      {
			CUtility.nyan(0 <= i);
			CUtility.nyan(m_spec.m_dtrans_ncols > i);
		      }
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.nyan((null == bunch.m_nfa_set 
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.nyan(nextstate < m_spec.m_dfa_states.size());
		      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.nyan(m_spec.m_dtrans_vector.size() == dfa.m_label);
		  }
  ...
  private void e_closure
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(null != bunch);
	    CUtility.nyan(null != bunch.m_nfa_set);
	    CUtility.nyan(null != bunch.m_nfa_bit);
	  }
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.nyan(bunch.m_nfa_bit.get(state.m_label));
	      }
	...
	while (false == nfa_stack.empty())
	  {
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.nyan(null != bunch.m_accept);
		    CUtility.nyan(CSpec.NONE == bunch.m_anchor
		...
		if (null != state.m_next)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.nyan(false == bunch.m_nfa_bit.get(state.m_next.m_label));
			  }
		...
		if (null != state.m_next2)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.nyan(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
			  }
...
class CUtility 
{
  ...
  static void nyan
	...
	if (DEBUG && false == expr)
	  {
	    System.out.println("nyanion Failed");
	    throw new Error("nyanion Failed.");
	  }
      }
...
class CLexGen 
{
  ...
  void generate
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(null != this);
	    CUtility.nyan(null != m_outstream);
	    CUtility.nyan(null != m_input);
	    CUtility.nyan(null != m_tokens);
	    CUtility.nyan(null != m_spec);
	    CUtility.nyan(m_init_flag);
	  }
  ...
  private char[] packCode
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(INIT_CODE == specified 
  ...
  private void userRules
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(END_OF_INPUT == m_spec.m_current_token);
	  }
	...
	for (rep_elem = 0; rep_elem < start_macro; ++rep_elem)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.nyan(rep_elem < replace.length);
	      }
	  }
  ...
  private void saveMacro
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan(0 < count_def);
	    CUtility.nyan(0 < count_name);
	    CUtility.nyan(null != m_spec.m_macros);
	  }
  ...
  private void saveStates
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.nyan('%' == m_input.m_line[0]);
	    CUtility.nyan('s' == m_input.m_line[1]);
	    CUtility.nyan(m_input.m_line_index <= m_input.m_line_read);
	    CUtility.nyan(0 <= m_input.m_line_index);
	    CUtility.nyan(0 <= m_input.m_line_read);
	  }
  ...
  private void details
	...
	while (names.hasMoreElements())
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.nyan(null != name);
		CUtility.nyan(null != def);
	      }
	...
	if (false == m_spec.m_count_chars)
	...
	else
	  {
	    if (CUtility.DEBUG)
	      {
		CUtility.nyan(m_spec.m_count_lines);
	      }
...
final class SparseBitSet implements Cloneable {
	...
	CUtility.nyan(size<bits.length);
	insert_block(idx, bnum);
    }
    private void insert_block(int idx, int bnum) {
	CUtility.nyan(idx<=size);
	CUtility.nyan(idx==size || offs[idx]!=bnum);
	...
	CUtility.nyan(!a.get(0) && !a.get(1));
	CUtility.nyan(!a.get(123329));
	a.set(0); CUtility.nyan(a.get(0) && !a.get(1));
	a.set(1); CUtility.nyan(a.get(0) && a.get(1));
	a.clearAll();
	CUtility.nyan(!a.get(0) && !a.get(1));
	...
	for (int n=0; n<ITER; n++) {
	    ...
	    CUtility.nyan(a.get(rr) && !a.get(rr+1) && !a.get(rr-1));
	    for (int i=0; i<v.size(); i++)
		CUtility.nyan(a.get(((Integer)v.elementAt(i)).intValue()));
	}
	SparseBitSet b = (SparseBitSet) a.clone();
	CUtility.nyan(a.equals(b) && b.equals(a));
	for (int n=0; n<ITER/2; n++) {
	    ...
	    CUtility.nyan(!b.get(m));
	}
	CUtility.nyan(!a.equals(b));
	SparseBitSet c = (SparseBitSet) a.clone();
	SparseBitSet d = (SparseBitSet) a.clone();
	c.and(a);
	CUtility.nyan(c.equals(a) && a.equals(c));
	c.xor(a);
	CUtility.nyan(!c.equals(a) && c.size()==0);
	d.or(b);
	CUtility.nyan(d.equals(a) && !b.equals(d));
	d.and(b);
	CUtility.nyan(!d.equals(a) && b.equals(d));
	d.xor(a);
	CUtility.nyan(!d.equals(a) && !b.equals(d));
